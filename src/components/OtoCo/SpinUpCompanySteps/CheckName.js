...
export default () => { 
    ...
    const handleJurisdictionChange = (e, data) => {
        ...
        dispatch({type: 'Enter Copyright Name on Welcome Board', value: compName})
    }
    ...
    const clickCheckHandler = async (e) => {
        ...
        dispatch({type: 'Enter Copyright title on Welcome Board', value: compName})
        if(!validate_input()) {
            dispatch({
                type: 'Show Error Msg on Welcome Board', 
                title: "Title is required!",
                msg: "Please enter the Copyright name you want to register!"
            });
            return;
        }
        ...
        .then(function({data}){
            if (data.results.total_count === 0) dispatch({type: 'Store CopyrightID'});
    ...
    const CheckNameForm = () => (
        <div>
            ...
            <Message negative style={{display: "none"}}>
                ...
                <p>Please Enter Another CopyrightID.</p>
            </Message>
            <Message negative style={{display: "none"}}>
                <Message.Header>Sorry! Please try again later.</Message.Header>
                <p>Search API service is busy.</p>
            </Message>
            <p className="normal-text">Accept the CopyrightID you want to registered.</p>
            <p className="normal-text">Click <b>`Check`</b> to verify.</p>
            <p className="normal-text">Click <b>`My Dashboard`</b> if you want to manage your Copyrights.</p>
            </div>
    ...
    const AvailableResult = () => (
        <div>
            <div style={{minHeight: '200px'}}>
                <p className="normal-text">Congrats! <b>{availableName}</b> your unique CopyrightID is ready for registration with the <b>{jurisdictionName}</b> State Registry.</p>
                <p className="normal-text">Your new Copyright will have its registered address at: <br/> <b>{jurisdictionStreet[jurisdictionSelected]}</b></p>
                { (jurisdictionSelected == 'us_wy') && <p className="bold-text">The full legal name is `<b>CopyrightShares WY LLC - {availableName} - Series #</b>` (the series number).</p> }
